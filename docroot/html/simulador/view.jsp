							...
							<div class="row">
								<div class="col-md-6">
									<ul class="nav nav-stacked">
										...
										<li>
											<button id="btn-meta" type="button" class="btn btn-block btn-info btn-xs" onclick="guardarMeta()" disabled>Guardar meta</button>
										</li>
									</ul>
								</div>
								<div class="col-md-6">
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-body">
        <p>Meta guardada correctamente</p>
      </div>
...
function ejecutarSimulacion(){
	...
	$.ajax({
		url: '<%=resourceURL%>',
		data:{'<portlet:namespace/>operacion':'simular',
			'<portlet:namespace/>productos':JSON.stringify(productosSimulacion),
...
function guardarMeta(){
	...
	$('.input-producto').each(function(i,inputProducto){
		...
		var valorInput=inputProducto.value!==""?inputProducto.value:0;
		var valorSimulacion=valorInput-numProductosGrupo;
		if(valorInput>numProductosGrupo){
			meta.push({c_categoria:inputProducto.id,i_num_productos:parseInt(valorInput)});
		}
	});
	
	$.ajax({
		url: '<%=resourceURL%>',
		data:{'<portlet:namespace/>operacion':'guardarRecomendacion',
			'<portlet:namespace/>meta':JSON.stringify(meta)},
		type:'POST',
		dataType:'json',
		success: function(respuesta) {
